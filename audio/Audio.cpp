/****************************************
 * @file Audio.cpp
 * @brief XAudio2を用いたサウンド管理モジュールの実装
 * @author Natsume Shidara
 * @date 2025/06/06
 *
 * WAV/MP3ファイルの読み込み・再生・停止・
 * ボリューム制御・ピッチ制御の実装。
 * MP3デコードにはMedia Foundationを使用。
 *
...
static int LoadMp3(const char* FileName, int index)
{
    ...
    hr = pReader->SetCurrentMediaType(static_cast<DWORD>(MF_SOURCE_READER_FIRST_AUDIO_STREAM), nullptr, pOutputType);
    pOutputType->Release();
...
// メモリからMP3読み込み（Media Foundation + IMFByteStream）

static int LoadMp3FromMemory(const void* pData, size_t dataSize, int index)
{
    if (!g_MFInitialized || !pData || dataSize == 0)
        return -1;
    ...
    void* pLock = GlobalLock(hGlobal);
    if (!pLock)
    {
        GlobalFree(hGlobal);
        return -1;
    }
    memcpy(pLock, pData, dataSize);
...
static int LoadWavFromMemory(const void* pData, size_t dataSize, int index)
{
    if (!pData || dataSize < 44) return -1; // WAVヘッダー最小サイズ

    const BYTE* ptr = static_cast<const BYTE*>(pData);

    // RIFF ヘッダ確認
    if (memcmp(ptr, "RIFF", 4) != 0) return -1;
    ptr += 4;
    ptr += 4; // ファイルサイズスキップ
    if (memcmp(ptr, "WAVE", 4) != 0) return -1;
    ptr += 4;

    WAVEFORMATEX wfx = {};
    const BYTE* audioPtr = nullptr;
    DWORD audioSize = 0;

    const BYTE* end = static_cast<const BYTE*>(pData) + dataSize;

    while (ptr + 8 <= end)
    {
        DWORD chunkId = *reinterpret_cast<const DWORD*>(ptr);
        ...
        if (chunkId == 0x20746D66) // "fmt "
        {
            if (chunkSize >= sizeof(PCMWAVEFORMAT))
            {
                if (chunkSize >= sizeof(WAVEFORMATEX))
                    memcpy(&wfx, ptr, sizeof(WAVEFORMATEX));
                else
                {
                    memset(&wfx, 0, sizeof(wfx));
                    memcpy(&wfx, ptr, chunkSize);
                }
            }
        }
    ...
    if (!audioPtr || audioSize == 0 || wfx.nBlockAlign == 0) return -1;

    g_Audio[index].SoundData = new BYTE[audioSize];
    memcpy(g_Audio[index].SoundData, audioPtr, audioSize);
    g_Audio[index].Length = static_cast<int>(audioSize);
    g_Audio[index].PlayLength = g_Audio[index].Length / wfx.nBlockAlign;
...
int LoadAudioFromMemory(const void* pData, size_t dataSize, const char* format)
{
    ...
    if (fmt == "mp3")
    {
        return LoadMp3FromMemory(pData, dataSize, index);
    }
    else if (fmt == "wav")
    {
        return LoadWavFromMemory(pData, dataSize, index);
    }
    else
    {
        // デフォルトはMP3として試す（ゲームのサウンドは大部分がMP3）
        return LoadMp3FromMemory(pData, dataSize, index);
    }
}
...
void SetAudioPitch(int Index, float ratio)
{
    ...
    // XAudio2の許容範囲にクランプ（XAUDIO2_MIN_FREQ_RATIO〜XAUDIO2_MAX_FREQ_RATIO）
    ratio = std::fmaxf(XAUDIO2_MIN_FREQ_RATIO, std::fminf(XAUDIO2_MAX_FREQ_RATIO, ratio));
    g_Audio[Index].SourceVoice->SetFrequencyRatio(ratio);
}