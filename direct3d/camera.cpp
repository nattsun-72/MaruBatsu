...
constexpr float DEFAULT_CAMERA_Z = -10.0f;                         // カメラ初期Z座標
constexpr float DEBUG_TEXT_Y_OFFSET = 32.0f;                       // デバッグテキストY座標オフセット
constexpr float DEBUG_TEXT_CHAR_SPACING = 16.0f;                   // デバッグテキスト文字間隔
constexpr float FOV_ADJUST_SPEED = 10.0f;                          // 視野角変更速度[度/秒]
constexpr float NEAR_CLIP_PLANE = 0.01f;                           // ニアクリップ面
constexpr float FAR_CLIP_PLANE = 10000.0f;                         // ファークリップ面
...
void Camera_Initialize()
{
    ...
    g_CameraPosition = { 0.0f, 1.0f, DEFAULT_CAMERA_Z };
    ...
    g_pDebugText = std::make_unique<hal::DebugText>(
        ...
        0.0f,                    // offsetX
        DEBUG_TEXT_Y_OFFSET,     // offsetY
        0,                       // maxLine (ULONG)
        0,                       // maxCharactersPerLine (ULONG)
        0.0f,                    // lineSpacing (float)
        DEBUG_TEXT_CHAR_SPACING  // characterSpacing (float)
    );
#endif
...
void Camera_Update(double elapsed_time)
{
    ...
    {
        g_Fov -= XMConvertToRadians(FOV_ADJUST_SPEED) * dt;
    }
    ...
    float nearZ = NEAR_CLIP_PLANE;
    float farZ = FAR_CLIP_PLANE;
