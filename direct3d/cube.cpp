...
// テクスチャアトラスのグリッド分割数
static constexpr float TEXTURE_ATLAS_DIVISIONS = 4.0f;
 ...
 *
 * テクスチャアトラスのUVを計算し、6面×2三角形の頂点バッファを作成する。
 */
void Cube_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    ...
    float w = 1.0f / TEXTURE_ATLAS_DIVISIONS; // 0.25
    float h = 1.0f / TEXTURE_ATLAS_DIVISIONS; // 0.25
...
void Cube_DrawShadow(const DirectX::XMMATRIX& world)
{
    ...
    // ※ ShaderShadowMap_Begin() / DepthStencilState / Topology は scene.cpp で設定済み
