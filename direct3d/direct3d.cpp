...
namespace {
    constexpr UINT SWAP_CHAIN_BUFFER_COUNT = 2;             // スワップチェーンのバックバッファ枚数
    constexpr float CLEAR_COLOR_R = 0.2f;                   // 画面クリア色（赤）
    constexpr float CLEAR_COLOR_G = 0.4f;                   // 画面クリア色（緑）
    constexpr float CLEAR_COLOR_B = 0.8f;                   // 画面クリア色（青）
    constexpr UINT BLEND_SAMPLE_MASK = 0xFFFFFFFF;          // ブレンドサンプルマスク（全チャンネル有効）
    constexpr INT SHADOW_DEPTH_BIAS = 1000;                 // シャドウマップ深度バイアス（固定）
    constexpr float SHADOW_SLOPE_SCALED_BIAS = 2.0f;        // シャドウマップ傾斜バイアス
}
...
// VSync制御
static UINT g_SyncInterval = 1;                                // Present同期間隔（1:VSync有効, 0:無効）
...
bool Direct3D_Initialize(HWND hWnd)
{
    ...
    swap_chain_desc.BufferCount = SWAP_CHAIN_BUFFER_COUNT;               // バックバッファ枚数
...
void Direct3D_Clear()
{
    float clear_color[4] = { CLEAR_COLOR_R, CLEAR_COLOR_G, CLEAR_COLOR_B, 1.0f }; // 背景色
...
/**
 * @brief 垂直同期（VSync）の有効/無効を設定
 */
void Direct3D_SetVSync(bool enable)
{
    g_SyncInterval = enable ? 1 : 0;
}
...
void Direct3D_SetBlendState(BlendMode mode)
{
    ...
    g_pDeviceContext->OMSetBlendState(pState, blendFactor, BLEND_SAMPLE_MASK);
}
...
HRESULT createRasterizerState()
{
    ...
    rd.CullMode = D3D11_CULL_BACK;
    rd.DepthBias = SHADOW_DEPTH_BIAS;            // 固定バイアス
    rd.SlopeScaledDepthBias = SHADOW_SLOPE_SCALED_BIAS; // 傾きバイアス
    rd.DepthBiasClamp = 0.0f;
