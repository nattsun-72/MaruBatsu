/****************************************
 * @file grid.cpp
 * @brief XZ平面デバッググリッド描画モジュール
 * @author Natsume Shidara
 * @date 2025/06/06
 *
 * X軸方向とZ軸方向のラインで構成されるグリッドを
 * 3D空間のXZ平面上に描画する。
 ****************************************/
...
void Grid_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
...
#ifdef USE_ASSET_DLL
    {
        ...
        if (AssetDLL_GetData(IDR_TEX_WHITE, &pData, &dataSize))
            g_GridTexId = Texture_LoadFromMemory(pData, dataSize, L"grid_white");
    }
#else
...
/** @brief グリッド終了処理（頂点バッファ解放） */
void Grid_Finalize()
{
    SAFE_RELEASE(g_pVertexBuffer);
}

/** @brief グリッドをラインリストで描画 */
void Grid_Draw()
{
    Shader3D_Begin();
