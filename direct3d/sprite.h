/****************************************
 * @file sprite.h
 * @brief 2Dスプライト描画モジュールのヘッダ
 * @author Natsume Shidara
 * @date 2025/06/06
 *
 * テクスチャID指定またはSRV直接指定による
 * 2Dスプライト描画インターフェースを提供する。
 * UV切り抜き・サイズ指定・回転に対応。
 ****************************************/
...
/** @brief スプライト初期化（動的頂点バッファ作成） */
void Sprite_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
/** @brief スプライト終了処理 */
void Sprite_Finalize(void);

/** @brief スプライト描画開始（2D投影行列・シェーダー設定） */
void Sprite_Begin();

/**
 * @brief  全スプライト描画に適用する画面オフセットを設定する（画面シェイク用）
 * @detail Sprite_Begin が 2D 投影行列へ反映するため、Prim・Text・3D投影描画の
 *         すべてが一様にずれる。シェイク終了時は (0,0) に戻すこと。
 * @param  x,y 画面ピクセル単位のオフセット
 */
