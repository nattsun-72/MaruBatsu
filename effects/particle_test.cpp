 ...
 * @date 2026/02/04
 * @update 2026/02/06 - 深度書き込み無効化、描画改善
 ****************************************/
#include "particle_test.h"
#include "billboard.h"
using namespace DirectX;

/** @brief 速度による移動とライフタイムに応じたフェードアウト */
void NormalParticle::Update(double elapsedTime)
{
    ...
    // 3. ライフタイムに応じてフェードアウト
    float lifeRatio = static_cast<float>(std::fmin(GetAccumulatedTime() / GetLifeTime(), 1.0));
...
void NormalParticle::Draw() const
{
    ...
    Billboard_DrawBatch(m_texId, position, { m_scale, m_scale }, { 0.0f, 0.0f }, color);

    
}

/** @brief 放射状・上方向バイアスのパーティクルを生成 */
