 ...
 * @date 2025/11/26
 * @update 2026/01/13 - EnemyFlying対応
 ****************************************/
...
{
    // 敵同士の衝突解決
    constexpr int ENEMY_COLLISION_ITERATIONS = 2;

    // 切断設定
    constexpr float MIN_VOLUME = 0.0001f;

    // 残骸タイマー（0.25秒待機 + 1秒消滅 = 1.25秒）
    ...
    // コンボスコア倍率（カウント毎の加算量）
    constexpr float COMBO_SCORE_MULTIPLIER_PER_COUNT = 0.1f;
    // コンボスコア倍率の上限
    constexpr float COMBO_SCORE_MULTIPLIER_MAX = 5.0f;

    // 敵ベクタの初期予約サイズ
    constexpr int ENEMY_VECTOR_RESERVE_SIZE = 64;
}
...
namespace
{
    ...
    /**
     * @brief 敵からRigidBodyを取得する（EnemyGround/EnemyFlying対応）
     * @return RigidBody*（PhysicsModelを持たない場合はnullptr）
     */
    RigidBody* GetEnemyRigidBody(Enemy* pEnemy)
    {
        if (auto* pGround = dynamic_cast<EnemyGround*>(pEnemy))
        {
            PhysicsModel* pm = pGround->GetPhysicsModel();
            return pm ? pm->GetRigidBody() : nullptr;
        }
        if (auto* pFlying = dynamic_cast<EnemyFlying*>(pEnemy))
        {
            PhysicsModel* pm = pFlying->GetPhysicsModel();
            return pm ? pm->GetRigidBody() : nullptr;
        }
        return nullptr;
    }
    ...
    /**
     * @brief 敵同士の衝突を解決する（めり込み防止）
     */
    void ResolveEnemyCollisions()
    {
        size_t count = g_Enemies.size();
        if (count < 2) return;

        for (int iter = 0; iter < ENEMY_COLLISION_ITERATIONS; ++iter)
        {
            for (size_t i = 0; i < count; ++i)
            {
                RigidBody* rbA = GetEnemyRigidBody(g_Enemies[i]);
                if (!rbA) continue;

                for (size_t j = i + 1; j < count; ++j)
                {
                    RigidBody* rbB = GetEnemyRigidBody(g_Enemies[j]);
                    if (!rbB) continue;

                    RigidBody::SolveCollision(rbA, rbB);
                }
            }
        }
    }
    ...
    /**
     * @brief 切断片の処理（敵として継続 or 残骸）
     * @param generationId 表裏ペア共通の衝突無視グループID
     */
        ...
        int generationId)
    {
        ...
        else
        {
            ...
            // 血しぶきエフェクト発生
            BloodEffect_Create(newPosition);

            // 撃破ボーナス（コンボボーナス適用）
            int comboCount = Combo_GetCount();
            float multiplier = 1.0f + comboCount * COMBO_SCORE_MULTIPLIER_PER_COUNT;
            multiplier = std::min(multiplier, COMBO_SCORE_MULTIPLIER_MAX);
...
{
    g_Enemies.clear();
    g_Enemies.reserve(ENEMY_VECTOR_RESERVE_SIZE);
...
bool Enemy_ProcessSliceResults()
{
    ...
    // 全結果を取り出し、敵以外はキューに戻す
    std::vector<SliceResult> nonEnemyResults;
    ...
    while (SliceTaskManager::TryGetCompletedResult(result))
    {
        ...
        {
            // 敵ではない — 後でキューに戻す
            nonEnemyResults.push_back(std::move(result));
            continue;
        }
        ...
        // Front側の処理（敵タイプを渡す）
        ProcessSlicedPiece(result.frontMeshes, result.originalModel, result, planeNormal, rootVolume, true, enemyType, genId);

        // Back側の処理（敵タイプを渡す）
        ProcessSlicedPiece(result.backMeshes, result.originalModel, result, planeNormal, rootVolume, false, enemyType, genId);
    ...
    // 敵以外の結果をキューに戻す（PropManagerが処理する）
    for (auto& r : nonEnemyResults)
    {
        SliceTaskManager::PushBackResult(std::move(r));
    }
