 ...
 * @date 2026/01/13
 * @update 2026/01/13 - サウンド対応・デバッグ描画
 ****************************************/
...
namespace
{
    ...
    // デバッグ描画：前方向線の長さ
    constexpr float DEBUG_FORWARD_LINE_LENGTH = 3.0f;

    // 角度変換（度→ラジアン）
    constexpr float DEG_TO_RAD = 3.14159f / 180.0f;

    // ホバー制御：バネ定数
    constexpr float HOVER_SPRING_CONSTANT = 5.0f;
    // ホバー制御：垂直方向の減衰率
    constexpr float HOVER_VERTICAL_DAMPING = 0.95f;

    // 最小移動距離（これ以下なら移動しない）
    constexpr float MIN_MOVE_DISTANCE = 0.1f;

    // ホバー移動速度倍率
    constexpr float HOVER_SPEED_MULTIPLIER = 10.0f;

    // 突進ターゲットのY軸オフセット
    constexpr float CHARGE_TARGET_Y_OFFSET = 1.0f;

    // 突進軌跡エフェクトパラメータ
    constexpr float CHARGE_TRAIL_COLOR_R = 1.0f;
    constexpr float CHARGE_TRAIL_COLOR_G = 0.3f;
    constexpr float CHARGE_TRAIL_COLOR_B = 0.1f;
    constexpr float CHARGE_TRAIL_COLOR_A = 0.8f;
    constexpr float CHARGE_TRAIL_SIZE = 0.5f;
    constexpr double CHARGE_TRAIL_LIFETIME = 0.3;

    // ワインドアップ軌跡エフェクトパラメータ
    constexpr float WINDUP_TRAIL_COLOR_R = 1.0f;
    constexpr float WINDUP_TRAIL_COLOR_G = 0.8f;
    constexpr float WINDUP_TRAIL_COLOR_B = 0.2f;
    constexpr float WINDUP_TRAIL_COLOR_A = 0.5f;
    constexpr float WINDUP_TRAIL_SIZE = 0.3f;
    constexpr double WINDUP_TRAIL_LIFETIME = 0.2;

    // 退避軌跡エフェクトパラメータ
    constexpr float RETREAT_TRAIL_COLOR_R = 0.5f;
    constexpr float RETREAT_TRAIL_COLOR_G = 0.3f;
    constexpr float RETREAT_TRAIL_COLOR_B = 1.0f;
    constexpr float RETREAT_TRAIL_COLOR_A = 0.6f;
    constexpr float RETREAT_TRAIL_SIZE = 0.4f;
    constexpr double RETREAT_TRAIL_LIFETIME = 0.25;

    // ホバー状態の最短待機時間
    constexpr float HOVER_MIN_DURATION = 0.2f;

    // ワインドアップ時の反動速度
    constexpr float WINDUP_RECOIL_SPEED = -3.0f;

    // 風切り音の再生間隔（秒）
    constexpr float WHOOSH_SOUND_INTERVAL = 0.3f;

    // 退避開始時の初期速度比率
    constexpr float RETREAT_INITIAL_SPEED_RATIO = 0.5f;

    // 退避時の移動速度倍率
    constexpr float RETREAT_SPEED_MULTIPLIER = 5.0f;

    // クールダウン時の速度減衰率
    constexpr float COOLDOWN_VELOCITY_DAMPING = 0.3f;

    // クールダウン時の旋回速度比率
    constexpr float COOLDOWN_CIRCLE_SPEED_RATIO = 0.5f;
}
...
void EnemyFlying_InitializeShared()
{
    ...
    {
        g_pEnemyFlyingModel = ModelLoad(MODEL_PATH, MODEL_SCALE, true, TEXTURE_PATH, TEXTURE_VERTICAL_PATH);
    }
}
...
namespace
{
    ...
    void DrawEnemyFlyingDebug(const PhysicsModel* pPhysics, const XMFLOAT4& color)
    {
        ...
        XMFLOAT3 lineEnd;
        XMStoreFloat3(&lineEnd, XMLoadFloat3(&pos) + forward * DEBUG_FORWARD_LINE_LENGTH);
...
EnemyFlying::EnemyFlying(const XMFLOAT3& position)
{
    ...
    m_HoverTimer = 0.0f;
    m_CircleAngle = static_cast<float>(rand() % 360) * DEG_TO_RAD;
...
void EnemyFlying::UpdateDistanceAndFacing()
{
    ...
    XMVECTOR toPlayerXZ = XMVectorSetY(toPlayer, 0.0f);
    if (XMVectorGetX(XMVector3LengthSq(toPlayerXZ)) > 0.01f)
    {
        XMVECTOR vFront = XMVector3Normalize(toPlayerXZ);
        XMStoreFloat3(&m_Front, vFront);

        // RigidBodyの回転にも反映（Y軸回転のみ）
        if (m_pPhysics)
        {
            float yaw = atan2f(m_Front.x, m_Front.z);
            XMFLOAT4 rot;
            XMStoreFloat4(&rot, XMQuaternionRotationRollPitchYaw(0.0f, yaw, 0.0f));
            m_pPhysics->GetRigidBody()->SetRotation(rot);
        }
    }
}
...
void EnemyFlying::ApplyHoverForce(float dt)
{
    ...
    float verticalForce = heightDiff * HOVER_SPRING_CONSTANT;

    XMFLOAT3 vel = GetVelocity();
    vel.y += verticalForce * dt;
    vel.y *= HOVER_VERTICAL_DAMPING;
...
void EnemyFlying::CircleAroundPlayer(float dt)
{
    ...
    MoveToward(target, HOVER_MOVE_SPEED * HOVER_SPEED_MULTIPLIER, dt);
}

void EnemyFlying::PrepareCharge()
{
    m_ChargeTarget = Player_GetPosition();
    m_ChargeTarget.y += CHARGE_TARGET_Y_OFFSET;
...
void EnemyFlying::ExecuteCharge(float dt)
{
    ...
    // 軌跡エフェクト
    XMFLOAT3 pos = GetPosition();
    Trail_Create(pos, { CHARGE_TRAIL_COLOR_R, CHARGE_TRAIL_COLOR_G, CHARGE_TRAIL_COLOR_B, CHARGE_TRAIL_COLOR_A }, CHARGE_TRAIL_SIZE, CHARGE_TRAIL_LIFETIME);
}
...
void EnemyFlying::StateHover::Update(float dt)
{
    ...
    if (m_StateTimer > HOVER_MIN_DURATION)  // 短い待機ですぐ攻撃に移行
    {
...
void EnemyFlying::StateWindup::Enter()
{
    ...
    XMVECTOR vVel = vDir * WINDUP_RECOIL_SPEED;
    XMStoreFloat3(&vel, vVel);
...
void EnemyFlying::StateWindup::Update(float dt)
{
    ...
    Trail_Create(pos, { WINDUP_TRAIL_COLOR_R, WINDUP_TRAIL_COLOR_G, WINDUP_TRAIL_COLOR_B, WINDUP_TRAIL_COLOR_A }, WINDUP_TRAIL_SIZE, WINDUP_TRAIL_LIFETIME);
...
void EnemyFlying::StateCharge::Update(float dt)
{
    ...
    whooshTimer += dt;
    if (whooshTimer >= WHOOSH_SOUND_INTERVAL)
    {
...
void EnemyFlying::StateRetreat::Enter()
{
    ...
    XMFLOAT3 vel;
    XMStoreFloat3(&vel, awayDir * RETREAT_SPEED * RETREAT_INITIAL_SPEED_RATIO);
    vel.y = RETREAT_SPEED * RETREAT_INITIAL_SPEED_RATIO;
    pOwner->SetVelocity(vel);
}
...
void EnemyFlying::StateRetreat::Update(float dt)
{
    ...
    pOwner->UpdateDistanceAndFacing();
    pOwner->MoveToward(m_RetreatTarget, RETREAT_SPEED * RETREAT_SPEED_MULTIPLIER, dt);
    pOwner->ClampToTerrain();

    XMFLOAT3 pos = pOwner->GetPosition();
    Trail_Create(pos, { RETREAT_TRAIL_COLOR_R, RETREAT_TRAIL_COLOR_G, RETREAT_TRAIL_COLOR_B, RETREAT_TRAIL_COLOR_A }, RETREAT_TRAIL_SIZE, RETREAT_TRAIL_LIFETIME);
...
void EnemyFlying::StateCooldown::Enter()
{
    ...
    EnemyFlying* pOwner = static_cast<EnemyFlying*>(m_pOwner);
    XMFLOAT3 vel = pOwner->GetVelocity();
    vel.x *= COOLDOWN_VELOCITY_DAMPING;
    vel.y *= COOLDOWN_VELOCITY_DAMPING;
    vel.z *= COOLDOWN_VELOCITY_DAMPING;
    pOwner->SetVelocity(vel);
}
...
void EnemyFlying::StateCooldown::Update(float dt)
{
    ...
    pOwner->GetCircleAngle() += EnemyFlyingConfig::CIRCLE_SPEED * COOLDOWN_CIRCLE_SPEED_RATIO * dt;
