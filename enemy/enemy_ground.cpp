...
void EnemyGround_InitializeShared()
{
    ...
    {
        g_pEnemyGroundModel = ModelLoad(MODEL_PATH, MODEL_SCALE,false, TEXTURE_PATH, TEXTURE_VERTICAL_PATH);
    }
}
...
{
    // デバッグ描画：前方向線の長さ
    constexpr float DEBUG_FORWARD_LINE_LENGTH = 2.0f;

    // 発射位置のY軸オフセット
    constexpr float FIRE_POSITION_Y_OFFSET = 0.5f;

    // 照準ターゲットのY軸オフセット（プレイヤー中心付近）
    constexpr float AIM_TARGET_Y_OFFSET = 1.0f;
...
void EnemyGround::UpdateDistanceAndFacing()
{
    ...
    {
        XMVECTOR vFront = XMVector3Normalize(toPlayer);
