...
public:
    ...
    /// 一度しか取得できないアビリティか。
    /// true のものは、所持済みなら報酬画面の抽選候補から除外される
    /// (盤面拡張など、2個目を取得しても効果が重複しないアビリティ)。
    bool        unique      = false;

    /// ボス固有報酬専用のアビリティか。
    /// true のものは通常の報酬抽選(3択)の候補から常に除外され、
    /// 対応ボスの撃破時にのみ獲得できる(氷駒・螺旋の欠片など)。
    bool        bossExclusive = false;

    /// 任意発動時にHUDへフラッシュ表示する文言 (空なら表示しない)。
    /// 例: 集中="+2:00"、螺旋の欠片="盤面が回転！"
