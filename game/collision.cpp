...
{
    // イプシロン値（浮動小数点比較用）
    constexpr float COLLISION_EPSILON = 1e-5f;          // 一般的な許容誤差
    constexpr float COLLISION_EPSILON_SQ = 1e-6f;       // 二乗距離・縮退判定用
    constexpr float COLLISION_EPSILON_TIGHT = 1e-8f;    // 高精度判定用

    // SAT（分離軸定理）軸数
    constexpr int SAT_OBB_TRI_AXIS_COUNT = 13;         // OBB vs 三角形: 面法線(3) + 面法線(1) + エッジ外積(9)
    constexpr int SAT_OBB_OBB_MAX_AXES = 15;           // OBB vs OBB: 面法線(6) + エッジ外積(9)

    // カプセル vs OBB サンプル数
    constexpr int CAPSULE_OBB_SAMPLE_COUNT = 5;        // カプセル球サンプリング数
}
...
static XMVECTOR ClosestPointOnSegmentToSegment(XMVECTOR p1, XMVECTOR q1, XMVECTOR p2, XMVECTOR q2)
{
    ...
    if (a <= COLLISION_EPSILON_SQ && e <= COLLISION_EPSILON_SQ) return p1;
    if (a <= COLLISION_EPSILON_SQ) return p1;

    float c = XMVectorGetX(XMVector3Dot(d1, r));
    if (e <= COLLISION_EPSILON_SQ) {
...
Hit Collision_IsHitOBBTriangle(const OBB& obb, const Triangle& tri)
{
    ...
    XMVECTOR axes[SAT_OBB_TRI_AXIS_COUNT];
    ...
    // 2. 分離軸テスト
    for (int i = 0; i < SAT_OBB_TRI_AXIS_COUNT; i++)
    {
        XMVECTOR axis = axes[i];
        if (XMVectorGetX(XMVector3LengthSq(axis)) < COLLISION_EPSILON_SQ) continue;
...
bool Collision_IntersectRayAABB(const Ray& ray, const AABB& aabb, float* outDist)
{
    ...
    // X
    if (std::abs(dir.x) < COLLISION_EPSILON_SQ) {
    ...
    // Y
    if (std::abs(dir.y) < COLLISION_EPSILON_SQ) {
    ...
    // Z
    if (std::abs(dir.z) < COLLISION_EPSILON_SQ) {
...
Hit Collision_IsHitOBBOBB(const OBB& a, const OBB& b)
{
    ...
    XMVECTOR axes[SAT_OBB_OBB_MAX_AXES];
    int axisCount = 0;
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            if (XMVectorGetX(XMVector3LengthSq(cross)) > COLLISION_EPSILON_SQ) {
    ...
    const XMFLOAT3& segEnd)
{
    ...
    float t = (denom > COLLISION_EPSILON_TIGHT) ? XMVectorGetX(XMVector3Dot(p - a, ab)) / denom : 0.0f;
    ...
    float* outS, float* outT)
{
    ...
    // 両方が点の場合
    if (a <= COLLISION_EPSILON_SQ && e <= COLLISION_EPSILON_SQ) {
        s = t = 0.0f;
        XMFLOAT3 result;
    ...
    // d1が点の場合
    if (a <= COLLISION_EPSILON_SQ) {
        s = 0.0f;
        t = std::clamp(f / e, 0.0f, 1.0f);
    }
    // d2が点の場合
    else if (e <= COLLISION_EPSILON_SQ) {
        t = 0.0f;
        float c = XMVectorGetX(XMVector3Dot(d1, r));
...
Hit Collision_IsHitCapsuleOBB(const Capsule& capsule, const OBB& obb)
{
    ...
    const int sampleCount = CAPSULE_OBB_SAMPLE_COUNT;
    Hit bestHit;
    bestHit.depth = -1.0f;
...
Hit Collision_IsHitCapsuleTriangle(const Capsule& capsule, const Triangle& tri)
{
    ...
    if (minDist < capsule.radius) {
        ...
        if (minDist > COLLISION_EPSILON_SQ) {
...
bool Collision_IntersectRayCapsule(const Ray& ray, const Capsule& capsule, float* outDist)
{
    ...
    if (axisLen < COLLISION_EPSILON_SQ) {
        // カプセルが球に退化している場合
        Sphere sphere;
