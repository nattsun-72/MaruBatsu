/****************************************
 * @file input_manager.cpp
 * @brief 入力管理（マウス状態の一元管理）
 * @author Natsume Shidara
 * @date 2026/01/07
 ****************************************/

#include "input_manager.h"

 // 内部変数
static Mouse_State g_MouseState = {};
static Mouse_State g_MouseStatePrev = {};

// 初期化
void InputManager_Initialize()
{
    g_MouseState = {};
    g_MouseStatePrev = {};
}

// 終了
void InputManager_Finalize()
{
    // 特になし
}

// 更新（フレームの最初に1回だけ呼ぶ）
void InputManager_Update()
{
    g_MouseStatePrev = g_MouseState;
    Mouse_GetState(&g_MouseState);
}

// マウス状態取得
const Mouse_State& InputManager_GetMouseState()
{
    return g_MouseState;
}

// マウスボタン トリガー判定
// 前フレームで押されていなくて、今フレームで押されている場合にtrue
bool InputManager_IsMouseLeftTrigger()
{
    return (g_MouseState.leftButton && !g_MouseStatePrev.leftButton);
}

bool InputManager_IsMouseRightTrigger()
{
    return (g_MouseState.rightButton && !g_MouseStatePrev.rightButton);
}
