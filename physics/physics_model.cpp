...
namespace {
    constexpr float MIN_COLLIDER_EXTENT = 0.01f;       // コライダー縮小時の最小値
    constexpr float GROUND_BOUNCE_DAMPING = 0.3f;       // 地面反発の減衰係数
    constexpr float MIN_BOUNCE_VELOCITY = 0.5f;         // 反発を無視する最小速度
    constexpr float FRICTION_EPSILON = 1e-6f;            // 摩擦計算の接線ベクトル閾値
}
    ...
    , m_colliderType(colliderType)
{
    ...
    {
        col.obb.extents.x = std::max(col.obb.extents.x - SKIN_WIDTH, MIN_COLLIDER_EXTENT);
        col.obb.extents.y = std::max(col.obb.extents.y - SKIN_WIDTH, MIN_COLLIDER_EXTENT);
        col.obb.extents.z = std::max(col.obb.extents.z - SKIN_WIDTH, MIN_COLLIDER_EXTENT);
    }
    else if (col.type == ColliderType::Sphere)
    {
        col.sphere.radius = std::max(col.sphere.radius - SKIN_WIDTH, MIN_COLLIDER_EXTENT);
    }
...
void PhysicsModel::Update(double elapsed_time)
{
    ...
    {
        m_glowTimer -= dt;
        if (m_glowTimer < 0.0f) m_glowTimer = 0.0f;
    }
    ...
    if (m_RigidBody.IsGravityEnabled())
    {
        ...
        if (actualBottom < terrainHeight)
        {
            ...
            if (vel.y < 0.0f)
            {
                vel.y *= -m_RigidBody.GetParams().restitution * GROUND_BOUNCE_DAMPING;
                // 小さな反発は無視
                if (fabsf(vel.y) < MIN_BOUNCE_VELOCITY) vel.y = 0.0f;
                m_RigidBody.SetVelocity(vel);
            }
        }
    }
    ...
    // ステージ壁との衝突判定
    const int wallCount = Stage_GetWallCount();
    for (int i = 0; i < wallCount; ++i)
    {
        const AABB& wallAABB = Stage_GetWallAABB(i);
        Hit hit = Collision_IsHitAABB(worldAABB, wallAABB);
...
void PhysicsModel::ApplyStaticCollisionResponse(const Hit& hit)
{
    ...
    if (tangentLenSq > FRICTION_EPSILON)
    {
