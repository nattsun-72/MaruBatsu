...
public:
    ...
    // ワールド空間のコライダー取得（スケール考慮済み）
    Collider GetWorldCollider() const { return m_RigidBody.GetWorldCollider(m_Scale); }
    ...
    // 断面グロー
    void SetGlowTimer(float time) { m_glowTimer = time; }
    float GetGlowTimer() const { return m_glowTimer; }
    float GetGlowIntensity() const {
        return (m_glowTimer > 0.0f) ? (m_glowTimer / GLOW_DURATION) : 0.0f;
    }

    // 定数（public）
    static constexpr float GLOW_DURATION = 2.0f;       // グロー持続時間（秒）
...
private:
    ...
    float m_glowTimer;                  // 断面グロー残り時間（0以下=グロー無し）
    bool m_isDead;                      // 消滅フラグ
