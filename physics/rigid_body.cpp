...
namespace {
    constexpr float ANGULAR_VELOCITY_EPSILON = 1e-8f;       // 角速度の有効判定閾値
    constexpr float PENETRATION_THRESHOLD = 0.5f;            // 位置補正のみ行う貫通深度閾値
    constexpr float LOW_VELOCITY_RESTITUTION_FACTOR = 0.5f;  // 低速衝突時の反発係数減衰率
    constexpr float TANGENT_LENGTH_EPSILON = 1e-6f;           // 接線ベクトル有効判定閾値
    constexpr float MASS_CHANGE_EPSILON = 1e-6f;              // 質量変更検知閾値
    constexpr float MIN_INERTIA_RADIUS = 0.01f;              // 慣性テンソル計算の最小半径
    constexpr float SPHERE_INERTIA_NUMERATOR = 2.0f;         // 球体慣性テンソル係数（分子）
    constexpr float SPHERE_INERTIA_DENOMINATOR = 5.0f;       // 球体慣性テンソル係数（分母）
    constexpr float BOX_INERTIA_DIVISOR = 12.0f;             // 直方体慣性テンソル係数（分母）
    constexpr float PARALLEL_AXIS_EPSILON = 1e-6f;            // 平行軸定理のオフセット閾値
    ...
    XMMATRIX ZeroMatrix()
    {
        return XMMATRIX(g_XMZero, g_XMZero, g_XMZero, g_XMZero);
    }
}
...
void RigidBody::Initialize(const XMFLOAT3& pos, const Collider& collider, float mass)
{
    ...
    // ワールド回転は常に単位クォータニオン（直立）で開始
    // OBBの向きはローカルコライダーに保持して衝突判定で使う
    XMStoreFloat4(&m_Rotation, XMQuaternionIdentity());
...
void RigidBody::Integrate(float dt)
{
    ...
    // 重力適用（拘束の前に加算して、拘束で正しくゼロ化できるようにする）
...
{
    float angVelLenSq = XMVectorGetX(XMVector3LengthSq(vAngVel));
    if (angVelLenSq > ANGULAR_VELOCITY_EPSILON)
    {
...
bool RigidBody::SolveCollision(RigidBody* rbA, RigidBody* rbB)
{
    ...
    if (hit.depth > PENETRATION_THRESHOLD)
    {
...
bool RigidBody::ApplyCollisionImpulse(RigidBody* rbA, RigidBody* rbB, const Hit& hit)
{
    ...
    float invMassA = pA.isKinematic ? 0.0f : SafeInverse(pA.mass);
    float invMassB = pB.isKinematic ? 0.0f : SafeInverse(pB.mass);
    float totalInvMass = invMassA + invMassB;
    if (totalInvMass <= 0.0f) return true;

    XMMATRIX invInertiaA = pA.isKinematic ? ZeroMatrix() : XMLoadFloat3x3(&rbA->m_InvInertiaTensorWorld);
    XMMATRIX invInertiaB = pB.isKinematic ? ZeroMatrix() : XMLoadFloat3x3(&rbB->m_InvInertiaTensorWorld);
    ...
    if (std::abs(velAlongNormal) < 1.0f) e *= LOW_VELOCITY_RESTITUTION_FACTOR;
    ...
    if (tangentLenSq > TANGENT_LENGTH_EPSILON)
    {
...
void RigidBody::ApplyPositionalCorrection(RigidBody* rbA, RigidBody* rbB, const Hit& hit, float totalInvMass)
{
    ...
    float invMassA = rbA->m_Params.isKinematic ? 0.0f : SafeInverse(rbA->m_Params.mass);
    float invMassB = rbB->m_Params.isKinematic ? 0.0f : SafeInverse(rbB->m_Params.mass);
...
{
    bool massChanged = std::abs(m_Params.mass - param.mass) > MASS_CHANGE_EPSILON;
    m_Params = param;
    if (massChanged)
    {
        ComputeLocalInertiaTensor();
...
// 内部更新
void RigidBody::ComputeLocalInertiaTensor()
{
    float Ixx = 0.0f, Iyy = 0.0f, Izz = 0.0f;
    XMFLOAT3 offset = { 0.0f, 0.0f, 0.0f };

    if (m_LocalCollider.type == ColliderType::Sphere)
    {
        offset = m_LocalCollider.sphere.center;
        const float r = std::max(m_LocalCollider.sphere.radius, MIN_INERTIA_RADIUS);
        const float I = (SPHERE_INERTIA_NUMERATOR / SPHERE_INERTIA_DENOMINATOR) * m_Params.mass * r * r;
        Ixx = Iyy = Izz = I;
    }
    else if (m_LocalCollider.type == ColliderType::Box)
    {
        offset = m_LocalCollider.obb.center;
        const float w = std::max(m_LocalCollider.obb.extents.x * 2.0f, MIN_INERTIA_RADIUS);
        const float h = std::max(m_LocalCollider.obb.extents.y * 2.0f, MIN_INERTIA_RADIUS);
        const float d = std::max(m_LocalCollider.obb.extents.z * 2.0f, MIN_INERTIA_RADIUS);
        const float coef = m_Params.mass / BOX_INERTIA_DIVISOR;
        Ixx = coef * (h * h + d * d);
        Iyy = coef * (w * w + d * d);
        Izz = coef * (w * w + h * h);
    }
    ...
    if (offsetLenSq > PARALLEL_AXIS_EPSILON)
    {
...
Collider RigidBody::GetWorldCollider(const XMFLOAT3& scale) const
{
    ...
    switch (worldCol.type)
    {
    case ColliderType::Sphere:
    {
        XMVECTOR vLocalCenter = XMLoadFloat3(&m_LocalCollider.sphere.center);
        // スケール適用後に回転・移動
        XMVECTOR vScaled = XMVectorMultiply(vLocalCenter, vScale);
        XMVECTOR vWorldCenter = XMVectorAdd(XMVector3Rotate(vScaled, qRot), vPos);
        XMStoreFloat3(&worldCol.sphere.center, vWorldCenter);
        // 半径はスケールの最大成分でスケーリング
        float maxScale = (std::max)({ scale.x, scale.y, scale.z });
        worldCol.sphere.radius = m_LocalCollider.sphere.radius * maxScale;
        break;
    }
    case ColliderType::Box:
    {
        ...
        // Extentsにスケール適用
        worldCol.obb.extents.x = m_LocalCollider.obb.extents.x * scale.x;
        worldCol.obb.extents.y = m_LocalCollider.obb.extents.y * scale.y;
        worldCol.obb.extents.z = m_LocalCollider.obb.extents.z * scale.z;
    ...
    {
        // AABBのローカル中心・サイズを取得
        XMFLOAT3 localCenter = m_LocalCollider.aabb.GetCenter();
        XMFLOAT3 localSize = m_LocalCollider.aabb.GetSize();

        // スケーリングした半サイズ
        float hw = localSize.x * 0.5f * scale.x;
        float hh = localSize.y * 0.5f * scale.y;
        float hd = localSize.z * 0.5f * scale.z;
        ...
        XMVECTOR vWorldCenter = XMVectorAdd(XMVector3Rotate(vScaled, qRot), vPos);
        XMFLOAT3 wc;
        XMStoreFloat3(&wc, vWorldCenter);

        worldCol.aabb.min = { wc.x - hw, wc.y - hh, wc.z - hd };
        worldCol.aabb.max = { wc.x + hw, wc.y + hh, wc.z + hd };
        break;
    }
    case ColliderType::Capsule:
    {
        XMVECTOR vLocalStart = XMVectorMultiply(XMLoadFloat3(&m_LocalCollider.capsule.start), vScale);
        XMVECTOR vLocalEnd   = XMVectorMultiply(XMLoadFloat3(&m_LocalCollider.capsule.end),   vScale);
        XMStoreFloat3(&worldCol.capsule.start,
            XMVectorAdd(XMVector3Rotate(vLocalStart, qRot), vPos));
        XMStoreFloat3(&worldCol.capsule.end,
            XMVectorAdd(XMVector3Rotate(vLocalEnd, qRot), vPos));
        // カプセル半径はXZ面のスケールで
        float maxScaleXZ = (std::max)(scale.x, scale.z);
        worldCol.capsule.radius = m_LocalCollider.capsule.radius * maxScaleXZ;
        break;
    }
    case ColliderType::Triangle:
    {
        XMVECTOR v0 = XMVectorMultiply(XMLoadFloat3(&m_LocalCollider.triangle.p0), vScale);
        XMVECTOR v1 = XMVectorMultiply(XMLoadFloat3(&m_LocalCollider.triangle.p1), vScale);
        XMVECTOR v2 = XMVectorMultiply(XMLoadFloat3(&m_LocalCollider.triangle.p2), vScale);
        XMStoreFloat3(&worldCol.triangle.p0, XMVectorAdd(XMVector3Rotate(v0, qRot), vPos));
        XMStoreFloat3(&worldCol.triangle.p1, XMVectorAdd(XMVector3Rotate(v1, qRot), vPos));
        XMStoreFloat3(&worldCol.triangle.p2, XMVectorAdd(XMVector3Rotate(v2, qRot), vPos));
        // 法線も回転（スケールは不要、正規化で吸収）
        XMVECTOR vNormal = XMLoadFloat3(&m_LocalCollider.triangle.normal);
        XMStoreFloat3(&worldCol.triangle.normal, XMVector3Normalize(XMVector3Rotate(vNormal, qRot)));
        break;
    }
    }
