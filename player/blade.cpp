...
namespace
{
    ...
    constexpr float IDLE_SWAY_Y_SPEED_RATIO = 0.7f;    // 待機揺れY軸速度の倍率
    constexpr float IDLE_SWAY_Y_AMOUNT_RATIO = 0.5f;  // 待機揺れY軸振幅の倍率
    constexpr float HORIZONTAL_STRIKE_START = 0.15f;   // 横なぎ斬撃フェーズ開始閾値
    constexpr float HORIZONTAL_STRIKE_END = 0.85f;     // 横なぎ斬撃フェーズ終了閾値
    constexpr float DEBUG_SPHERE_RADIUS = 0.05f;       // デバッグ用球体の半径

#ifdef _DEBUG
    constexpr float DEBUG_ADJUST_SPEED = 0.02f;
    constexpr int DEBUG_MODE_COUNT = 2;                // デバッグモードの切替数
#endif
}
...
// X字交互選択（袈裟 ↔ 逆袈裟）
static bool g_SwingFromLeft = true;

static int SelectNextPattern()
{
    // 袈裟(0): 左上→右下, 逆袈裟(1): 右上→左下
    int next = g_SwingFromLeft ? 0 : 1;
    g_SwingFromLeft = !g_SwingFromLeft;
...
static void UpdateState_Idle(float dt)
{
    ...
    g_LocalPosition.y = cosf(g_IdleSwayTime * IDLE_SWAY_Y_SPEED_RATIO) * IDLE_SWAY_AMOUNT * IDLE_SWAY_Y_AMOUNT_RATIO;
...
static void UpdateState_HorizontalSlash(float dt)
{
    ...
    if (t > HORIZONTAL_STRIKE_START && t < HORIZONTAL_STRIKE_END)
    {
        PerformSlice({ 0.0f, 1.0f, 0.0f }, HORIZONTAL_SLICE);
    }
...
static void DebugUpdate()
{
    ...
    {
        g_DebugMode = (g_DebugMode + 1) % DEBUG_MODE_COUNT;
    }
...
static void PerformSlice(const XMFLOAT3& sliceNormal, const SliceParams& params)
{
    ...
    float effectiveRayLength   = params.rayLength   * Fever_GetRangeMultiplier();
    float effectivePlaneSpread = params.planeSpread * Fever_GetRangeMultiplier();

    XMFLOAT3 dir1 = {
        rayDir.x * effectiveRayLength,
        rayDir.y * effectiveRayLength,
        rayDir.z * effectiveRayLength
    };

    // dir2のオフセット方向: cross(sliceNormal, dir1)を使うことで
    // cross(dir1, dir2) が sliceNormal と一致するようにする
    // （PropManager_TrySliceがcross(startRay, endRay)で法線を算出するため）
    XMVECTOR vDir1 = XMLoadFloat3(&dir1);
    XMVECTOR vSliceNormal = XMLoadFloat3(&sliceNormal);
    XMVECTOR vPerpendicular = XMVector3Normalize(XMVector3Cross(vSliceNormal, vDir1));
    XMFLOAT3 perp;
    XMStoreFloat3(&perp, vPerpendicular);

    XMFLOAT3 dir2 = {
        dir1.x + perp.x * effectivePlaneSpread,
        dir1.y + perp.y * effectivePlaneSpread,
        dir1.z + perp.z * effectivePlaneSpread
    };
    ...
    EnemyBullet_TrySlice(endRay, sliceNormal);
}
...
void Blade_DebugDraw()
{
    ...
    DebugRenderer::DrawSphere({ rayOrigin, DEBUG_SPHERE_RADIUS }, { 1.0f, 1.0f, 0.0f, 1.0f });
    DebugRenderer::DrawSphere({ rayEnd, DEBUG_SPHERE_RADIUS }, { 1.0f, 0.0f, 0.0f, 1.0f });
...
{
    return g_State == BladeState::FixedAttack ||
