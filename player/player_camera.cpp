...
namespace
{
    ...
    constexpr float EYE_HEIGHT = 1.6f;                   // プレイヤー目の高さオフセット
    constexpr float FOV_INTERP_SPEED = 10.0f;            // FOV補間速度のデフォルト値
    constexpr float MOUSE_SENSITIVITY_MULTIPLIER = 0.005f; // マウス感度の変換係数
    constexpr float FOV_EPSILON = 0.001f;                // FOV補間の収束判定閾値
    constexpr float NEAR_CLIP = 0.01f;                   // ニアクリップ面の距離
    constexpr float FAR_CLIP = 10000.0f;                 // ファークリップ面の距離
    constexpr float MIN_MOUSE_SENSITIVITY = 0.0001f;     // マウス感度の最小値
}
...
static XMFLOAT3 g_EyeOffset = { 0.0f, EYE_HEIGHT, 0.0f }; // プレイヤー頭の高さ
...
static float g_FOVSpeed = FOV_INTERP_SPEED;
...
void PLCamera_Initialize(XMFLOAT3 eyeOffset)
{
    ...
    g_FOVSpeed = FOV_INTERP_SPEED;

    // 感度初期化
    g_MouseSensitivity = GameSettings_GetSensitivity() * MOUSE_SENSITIVITY_MULTIPLIER;
    g_FirstUpdate = true;
...
void PLCamera_Update(double elapsed_time)
{
    ...
    // FOV補間
    //--------------------------------------
    if (g_FOVSpeed > 0.0f && fabsf(g_CurrentFOV - g_TargetFOV) > FOV_EPSILON)
    {
...
{
    float targetFOV = Fever_IsActive() ? Fever_GetFOVTarget() : DEFAULT_FOV;
    PLCamera_SetTargetFOV(targetFOV, speed);
}
...
{
    g_MouseSensitivity = fmaxf(MIN_MOUSE_SENSITIVITY, sensitivity);
}
