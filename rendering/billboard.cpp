...
// バッチ描画: キャッシュ用
static XMMATRIX g_CachedBillboardMatrix;
...
void Billboard_BeginBatch()
{
    ...
    // ビルボード行列をキャッシュ（1回だけ）
    g_CachedBillboardMatrix = GetBillboardMatrix();
}

void Billboard_DrawBatch(int texId, const DirectX::XMFLOAT3& position, const XMFLOAT2& scale, const XMFLOAT2& pivot, const XMFLOAT4& color)
{
    Shader_Billboard_SetUVParameter({ { 1.0f, 1.0f }, { 0.0f, 0.0f } });
    Shader_Billboard_SetColor(color);
    Texture_SetTexture(texId);
    ...
    XMMATRIX pivotOffset = XMMatrixTranslation(-pivot.x, -pivot.y, 0.0f);
    XMMATRIX matTranslation = XMMatrixTranslation(position.x, position.y, position.z);
    XMMATRIX matWorld = matScale * pivotOffset * g_CachedBillboardMatrix * matTranslation;
...
// バッチ描画（テクスチャ切り抜き対応版）
void Billboard_DrawBatch(int texId, const DirectX::XMFLOAT3& position, const DirectX::XMFLOAT2& scale, const DirectX::XMFLOAT4& tex_cut, const DirectX::XMFLOAT2& pivot, const XMFLOAT4& color)
{
    float uv_x = static_cast<float>(tex_cut.x / Texture_Width(texId));
    float uv_y = static_cast<float>(tex_cut.y / Texture_Height(texId));
    float uv_w = static_cast<float>(tex_cut.z / Texture_Width(texId));
