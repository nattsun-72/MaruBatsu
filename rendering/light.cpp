...
{
    // --- 環境光デフォルト値 ---
    constexpr float DEFAULT_AMBIENT = 0.2f;              // 環境光の初期強度（RGB共通）

    // --- 鏡面反射デフォルト値 ---
    constexpr float DEFAULT_SPECULAR_EYE_Z = -5.0f;     // カメラ初期位置（Z軸）
    constexpr float DEFAULT_SPECULAR_POWER = 32.0f;      // 鏡面反射の光沢度

    // --- 点光源制限 ---
    constexpr UINT MAX_POINT_LIGHTS = 4U;                // 点光源の最大数
}
...
static AmbientCB g_ambientData = { XMFLOAT4(DEFAULT_AMBIENT, DEFAULT_AMBIENT, DEFAULT_AMBIENT, 1.0f) };
...
static SpecularLightCB g_specularData = {
    ...
    XMFLOAT3(0.0f, 0.0f, DEFAULT_SPECULAR_EYE_Z), // カメラ位置（初期値）
    DEFAULT_SPECULAR_POWER                        // 光沢度
};
...
void Light_SetPointLight(const PointLightCB* lights, int count)
{
    ...
    g_pointLightData.pointLightCount = min(safeCount, MAX_POINT_LIGHTS);
...
{
    /* 最大数に達している場合は何もしない */
    if (g_pointLightData.pointLightCount >= MAX_POINT_LIGHTS)
    {
        return;
    }
