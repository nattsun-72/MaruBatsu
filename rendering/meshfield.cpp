...
namespace
{
    ...
    // --- ハッシュ関数用素数 ---
    constexpr int HASH_PRIME_X = 73856093;               // X座標用ハッシュ素数
    constexpr int HASH_PRIME_Z = 19349663;               // Z座標用ハッシュ素数
    constexpr int HASH_POSITIVE_MASK = 0x7FFFFFFF;       // 正の値にするためのビットマスク
    constexpr int HASH_MODULO = 10000;                   // ハッシュ値の範囲

    // --- ノイズ地形パラメータ ---
    constexpr float NOISE_SCALE = 0.08f;                 // ノイズのスケール係数
    constexpr int FBM_OCTAVES = 4;                       // フラクタルブラウン運動のオクターブ数

    // --- 丘の波形パラメータ ---
    constexpr float HILL1_FREQ_X = 0.15f;               // 丘1のX方向周波数
    constexpr float HILL1_FREQ_Z = 0.12f;               // 丘1のZ方向周波数
    constexpr float HILL1_AMPLITUDE = 0.5f;              // 丘1の振幅
    constexpr float HILL2_FREQ_X = 0.08f;               // 丘2のX方向周波数
    constexpr float HILL2_OFFSET_X = 1.5f;              // 丘2のX方向位相オフセット
    constexpr float HILL2_FREQ_Z = 0.1f;                // 丘2のZ方向周波数
    constexpr float HILL2_OFFSET_Z = 0.8f;              // 丘2のZ方向位相オフセット
    constexpr float HILL2_AMPLITUDE = 0.3f;              // 丘2の振幅

    // --- 中央平坦化パラメータ ---
    constexpr float CENTER_FLATTEN_RATE = 0.002f;        // 中央付近の平坦化率

    // --- 地形頂点色（高さに応じた色） ---
    constexpr float LOW_COLOR_R = 0.35f;                 // 低地の赤成分
    constexpr float LOW_COLOR_G = 0.38f;                 // 低地の緑成分
    constexpr float LOW_COLOR_B = 0.45f;                 // 低地の青成分
    constexpr float HIGH_COLOR_R = 0.55f;                // 高地の赤成分
    constexpr float HIGH_COLOR_G = 0.55f;                // 高地の緑成分
    constexpr float HIGH_COLOR_B = 0.6f;                 // 高地の青成分

    // --- UVタイリング ---
    constexpr float UV_TILING = 0.5f;                    // UVの繰り返し係数
}
...
namespace Noise
{
    ...
    {
        int ix = static_cast<int>(floorf(x)) * HASH_PRIME_X;
        int iz = static_cast<int>(floorf(z)) * HASH_PRIME_Z;
        int hash = (ix ^ iz) & HASH_POSITIVE_MASK;
        return static_cast<float>(hash % HASH_MODULO) / static_cast<float>(HASH_MODULO);
    }
...
static float GenerateTerrainHeight(float x, float z)
{
    ...
    // 基本のノイズ地形
    float height = Noise::FBM(x * NOISE_SCALE, z * NOISE_SCALE, FBM_OCTAVES, 0.5f, 2.0f);

    // 0～1 を MIN_HEIGHT～MAX_HEIGHT に変換
    height = MIN_HEIGHT + height * (MAX_HEIGHT - MIN_HEIGHT);

    // 丘の追加（正弦波を組み合わせ）
    float hill1 = sinf(x * HILL1_FREQ_X) * cosf(z * HILL1_FREQ_Z) * HILL1_AMPLITUDE;
    float hill2 = sinf(x * HILL2_FREQ_X + HILL2_OFFSET_X) * sinf(z * HILL2_FREQ_Z + HILL2_OFFSET_Z) * HILL2_AMPLITUDE;
    height += hill1 + hill2;

    // 中央付近を少し低くする（アリーナ感）
    float distFromCenter = sqrtf(x * x + z * z);
    float centerFlatten = 1.0f - expf(-distFromCenter * distFromCenter * CENTER_FLATTEN_RATE);
...
static XMFLOAT4 GetHeightColor(float height)
{
    ...
    XMFLOAT4 lowColor = { LOW_COLOR_R, LOW_COLOR_G, LOW_COLOR_B, 1.0f };
    XMFLOAT4 highColor = { HIGH_COLOR_R, HIGH_COLOR_G, HIGH_COLOR_B, 1.0f };
...
static void CreateVertices()
{
    ...
    for (int z = 0; z < MESH_V_VERTEX_COUNT; z++)
    {
        for (int x = 0; x < MESH_H_VERTEX_COUNT; x++)
        {
            ...
            g_MeshVertex[index].uv = { x * UV_TILING, z * UV_TILING };  // UVタイリング
...
void MeshField_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
...
#ifdef USE_ASSET_DLL
    {
        ...
        size_t dataSize = 0;
        if (AssetDLL_GetData(IDR_TEX_FLOOR, &pData, &dataSize))
            g_MeshTexId = Texture_LoadFromMemory(pData, dataSize, L"meshfield_floor");
    }
#else
...
void MeshField_Draw(const XMMATRIX& mtxWorld)
{
    ...
    // ※ Shader3D_Begin() / DepthStencilState / Topology は scene.cpp で設定済み
