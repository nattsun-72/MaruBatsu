...
struct MeshData
{
    ...
    // 断面テクスチャブレンド比率（-1 = 通常メッシュ / 0.0 = 横切りのみ / 1.0 = 縦切りのみ / 中間 = ブレンド）
    float sliceBlendRatio = -1.0f;
};
...
struct MODEL
{
    ...
    // 断面テクスチャ（横切り = 水平方向の切断面）
    ID3D11ShaderResourceView* pSliceTextureSRV = nullptr;
    int sliceTextureId = -1;

    // 断面テクスチャ（縦切り = 垂直方向の切断面）
    // 未設定時は横切りテクスチャで代用
    ID3D11ShaderResourceView* pSliceVerticalSRV = nullptr;
    int sliceVerticalTextureId = -1;
    ...
    std::vector<DirectX::XMFLOAT4> materialColors;    // マテリアルごとのディフューズカラー
    ...
    void SetSliceVerticalTexturId(int texId){ sliceVerticalTextureId = texId;}
    const int& GetSliceVerticalTexturId() const {return sliceVerticalTextureId;}

    // 描画時に使用する実効SRVを取得（縦テクスチャ未設定時は横で代用）
    ID3D11ShaderResourceView* GetEffectiveVerticalSRV() const {
        return pSliceVerticalSRV ? pSliceVerticalSRV : pSliceTextureSRV;
    }
};
...
/**
 * @brief モデルシステムの初期化（フォールバックテクスチャ等）
 * @detail Scene_Initialize 等のシステム初期化時に1回呼ぶこと。
 *         ModelLoad の前に呼ばれていなくても内部でフォールバック初期化するが、
 *         明示的に呼ぶのが望ましい。
 */
...
/**
 * @brief モデルシステムの終了処理
 * @detail フォールバックテクスチャの解放とキャッシュクリアを行う。
 */
void Model_Finalize();
 ...
 */
MODEL* ModelLoad(const char* FileName, float scale = 1.0f, bool RHFlg = false, const wchar_t* pSlisedTextureFilename = nullptr, const wchar_t* pSliceVerticalFilename = nullptr);
