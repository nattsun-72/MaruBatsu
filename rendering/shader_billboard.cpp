 ...
 * @brief ビルボード用シェーダーの実装
 * @author Natsume Shidara
 * @date 2025/11/14
 *
 * CSOファイルまたはDLLリソースからシェーダーを読み込み、
 * 入力レイアウト・定数バッファを構築する。
 ****************************************/
...
bool Shader_Billboard_Initialize()
{
    ...
    // -----------------------------
    {
        ...
        if (!AssetDLL_GetData(IDR_SHADER_VS_BILLBOARD, &vsData, &vsSize) || !vsData)
        {
    ...
    // -----------------------------
    {
        ...
        size_t psSize = 0;
#ifdef USE_ASSET_DLL
        if (!AssetDLL_GetData(IDR_SHADER_PS_BILLBOARD, &psData, &psSize) || !psData)
        {
