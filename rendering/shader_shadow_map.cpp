...
static HRESULT CompileShaderFromMemory(const void* pSrcData, size_t srcDataSize, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
    ...
    HRESULT hr = D3DCompile(pSrcData, srcDataSize, "shadow_map.hlsl", nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE,
...
bool ShaderShadowMap_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
...
#ifdef USE_ASSET_DLL
    {
        ...
        size_t hlslSize = 0;
        if (!AssetDLL_GetData(IDR_SHADER_HLSL_SHADOW_MAP, &pHlslData, &hlslSize))
        {
            LogHR("DLL GetData shadow_map.hlsl", E_FAIL);
            return false;
        }
...
#else
    {
        const WCHAR* hlslPath = L"assets/shader/shadow_map.hlsl";
...
#ifdef USE_ASSET_DLL
    {
        ...
        if (!AssetDLL_GetData(IDR_SHADER_HLSL_SHADOW_MAP, &pHlslData, &hlslSize))
        {
            LogHR("DLL GetData shadow_map.hlsl (PS)", E_FAIL);
            return false;
        }
