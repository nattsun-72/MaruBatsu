...
{
public:
    // workerThreadCount = 0 の場合、PCの論理コア数から安全なスレッド数を自動決定
    static void Initialize(int workerThreadCount = 0);
    static void Finalize();

    // PCの論理コア数に基づいて安全なワーカースレッド数を算出
    static int CalcOptimalWorkerCount();
    ...
    // 結果をキューに戻す（他モジュール宛ての結果を返却）
    static void PushBackResult(SliceResult&& result);
