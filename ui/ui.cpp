...
namespace UIConfig
{
    ...
    // 空ハートの色・アルファ
    constexpr float EMPTY_HEART_COLOR = 0.5f;      // 空ハートの色成分値
    constexpr float EMPTY_HEART_ALPHA = 0.5f;      // 空ハートのアルファ値

    // 低HPしきい値
    constexpr int LOW_HP_THRESHOLD = 3;             // 低HP演出の発動しきい値
}
...
void UI_Initialize()
{
...
#ifdef USE_ASSET_DLL
    {
        ...
        size_t dataSize = 0;
        if (AssetDLL_GetData(IDR_TEX_UI_HEART_FULL, &pData, &dataSize))
            g_TexHeartFull = Texture_LoadFromMemory(pData, dataSize, L"ui_heart_full");
    }
    {
        const void* pData = nullptr;
        size_t dataSize = 0;
        if (AssetDLL_GetData(IDR_TEX_UI_HEART_EMPTY, &pData, &dataSize))
            g_TexHeartEmpty = Texture_LoadFromMemory(pData, dataSize, L"ui_heart_empty");
    }
#else
...
void UI_Draw()
{
    ...
    DirectX::XMFLOAT4 colorEmpty = { EMPTY_HEART_COLOR, EMPTY_HEART_COLOR, EMPTY_HEART_COLOR, EMPTY_HEART_ALPHA };
    ...
    for (int i = 0; i < MAX_HEARTS; i++)
    {
        ...
        if (i < currentHP && currentHP <= LOW_HP_THRESHOLD)
        {
            size *= pulseScale;
            // 中心を維持するためにオフセット
