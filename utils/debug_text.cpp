/****************************************
 * @file debug_text.cpp
 * @brief Direct3D11用デバッグテキスト表示の実装
 * @author Youhei Sato
 * @date 2025/06/15
 *
 * フォントアトラスからASCII文字を切り出し、
 * 頂点バッファで描画する。
 ****************************************/
...
namespace {
	constexpr int FONT_ATLAS_GRID_SIZE = 16;              // フォントアトラスのグリッドサイズ（16x16文字）
	constexpr float FONT_ATLAS_GRID_SIZE_F = 16.0f;       // フォントアトラスのグリッドサイズ（浮動小数点）
	constexpr int TAB_STOP_WIDTH = 4;                      // タブストップ幅（文字数）
	constexpr UINT BLEND_SAMPLE_MASK = 0xffffffff;         // ブレンドサンプルマスク（全チャンネル有効）
}
...
namespace hal
{
		...
		: m_pDevice(pDevice), m_pContext(pContext), m_FileName(pFontTextureFileName), m_OffsetX(offsetX), m_OffsetY(offsetY), m_MaxLine(maxLine), m_MaxCharactersPerLine(maxCharactersPerLine), m_LineSpacing(lineSpacing), m_CharacterSpacing(characterSpacing)
	{
...
#ifdef USE_ASSET_DLL
			// DLLリソースからフォントテクスチャ読み込み
			{
				int fontResId = -1;
				std::wstring fn(pFontTextureFileName);
				if (fn.find(L"consolab") != std::wstring::npos) fontResId = IDR_TEX_CONSOLAB;

				const void* pData = nullptr;
				size_t dataSize = 0;
				if (fontResId >= 0 && AssetDLL_GetData(fontResId, &pData, &dataSize))
				{
					if (FAILED(CreateWICTextureFromMemory(pDevice,
						static_cast<const uint8_t*>(pData), dataSize, &m_pTexture, &m_pTextureView)))
					{
						MessageBoxW(nullptr, L"フォントテクスチャの読み込みに失敗しました (DLL)", pFontTextureFileName, MB_OK | MB_ICONERROR);
						return;
					}
				}
				else
				{
					MessageBoxW(nullptr, L"フォントテクスチャリソースが見つかりません", pFontTextureFileName, MB_OK | MB_ICONERROR);
					return;
				}
			}
#else
		...
		if (!m_LineSpacing) {
			m_LineSpacing = m_TextureHeight / FONT_ATLAS_GRID_SIZE_F; // 16文字分の高さを1行の高さとする
		}

		if (!m_CharacterSpacing) {
			m_CharacterSpacing = m_TextureWidth / FONT_ATLAS_GRID_SIZE_F; // 16文字分の幅を1文字の幅とする
		}
	...
	{		
		...
		for(; *pText; pText++) {
			...
			if (*pText == '\n') {
				...
				while (m_TextLines.back().characterCount % TAB_STOP_WIDTH != 0) {
	...
	void DebugText::Draw()
	{
		...
		const float characterWidth = m_TextureWidth / FONT_ATLAS_GRID_SIZE_F;
		const float characterHeight = m_TextureHeight / FONT_ATLAS_GRID_SIZE_F;
		...
		for (const auto& strings : m_TextLines) {
			...
			for (const auto& string : strings.strings) {
				...
				for (const auto& code : string.characters) {
					...
					if (index) {
						float u0 = (index % FONT_ATLAS_GRID_SIZE) / FONT_ATLAS_GRID_SIZE_F;
						float v0 = (index / FONT_ATLAS_GRID_SIZE) / FONT_ATLAS_GRID_SIZE_F;
						float u1 = (index % 16 + 1) / FONT_ATLAS_GRID_SIZE_F;
						float v1 = (index / 16 + 1) / FONT_ATLAS_GRID_SIZE_F;
		...
		m_pContext->OMSetBlendState(m_pBlendState.Get(), blend_factor, BLEND_SAMPLE_MASK);
		...
		m_pContext->OMSetBlendState(pPreviousBlendState.Get(), previous_blend_factor, BLEND_SAMPLE_MASK);
