 ...
 * @brief 高分解能パフォーマンスカウンタによるシステムタイマーの実装
 * @author Youhei Sato
 * @date 2018/06/17
 *
 * QueryPerformanceCounterを使用した
 * 高精度タイマーの初期化・計測・制御。
 ****************************************/
#include <Windows.h>

namespace {
    constexpr int TIMER_ADVANCE_DIVISOR = 10; // タイマー進行量の分母（0.1秒分）
}
...
{
    g_StopTime += g_TicksPerSec / TIMER_ADVANCE_DIVISOR;
}
